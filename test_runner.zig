...
pub fn main() !void {
    ...
    for (builtin.test_functions) |t| {
        ...
        const is_not_timed = isNotTimed(t);
        ...
        var ns_taken: u64 = undefined;
        if (!is_not_timed) {
            ns_taken = slowest.endTiming(friendly_name);
        }
        ...
        if (env.verbose and !is_not_timed) {
            const ms = @as(f64, @floatFromInt(ns_taken)) / 1_000_000.0;
            printer.status(status, "{s} ({d:.2}ms)\n", .{ friendly_name, ms });
        } else {
            printer.status(status, "{s} (not timed)\n", .{friendly_name});
        }
    }
